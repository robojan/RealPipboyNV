...
char * GetExtraDataValue(BSExtraData* traverse)
{
	...
	switch (traverse->type) {			
		...
		case	kExtraData_Script                   	: 
			...
			if (pXScript->script)
				sprintf_s(buffer, sizeof(buffer), "script:[%#10X] eventList:[%#10X]  flags:{%#10x}", pXScript->script->refID, 
					(unsigned int)pXScript->eventList, (unsigned int)pXScript->script->flags);
			else
				sprintf_s(buffer, sizeof(buffer), "script:[%#10X] eventList:[%#10X]  flags:{%#10x}", (unsigned int)pXScript->script, 
					(unsigned int)pXScript->eventList, pXScript->script->flags);
			return buffer;
			break;
		case	kExtraData_Action                   	: 
			pXAction = (ExtraAction*)traverse;
			if (pXAction->actionRef && pXAction->actionRef->GetFullName())
				sprintf_s(buffer, sizeof(buffer), "{%#2X} [%#10X] (%s)", pXAction->flags0C, (unsigned int)pXAction->actionRef->refID, 
					pXAction->actionRef->GetFullName()->name.CStr());
			else
		...
		case	kExtraData_ReferencePointer         	: 
			...
			if (refr && refr->GetFullName())
				sprintf_s(buffer, sizeof(buffer), "[%#10X] (%s) [%#10X]", refr->refID, 
					refr->GetFullName()->name.CStr(), (unsigned int)refr->extraDataList.m_data);
			else
				sprintf_s(buffer, sizeof(buffer), "[%#10X] () [%#10X]", refr->refID, (unsigned int)refr->extraDataList.m_data);
			return buffer; break;
