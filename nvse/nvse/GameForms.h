...
STATIC_ASSERT(sizeof(EffectItemList) == 0x010);
...
*/
	...
	virtual UInt32	GetActorValueBaseI(UInt32 actorValueCode);	// GetBaseActorValue (used from Eval) result in EAX
	virtual float	GetActorValueBase(UInt32 actorValueCode);	// GetBaseActorValue internal, result in st
	virtual UInt32	GetActorValueI(UInt32 actorValueCode);	// GetActorValue internal, result in EAX
	virtual float	GetActorValue(UInt32 actorValueCode);	// GetActorValue (used from Eval) result in EAX
	virtual float	getAVModifier(UInt32 actorValueCode);	// GetBaseActorValue04 (internal) result in st
	virtual float	Fn_05(void * arg);
	virtual float	Fn_06(UInt32 actorValueCode);	// GetDamageActorValue or GetModifiedActorValue		called from Fn_08, result in st, added to Fn_01
	virtual UInt32	GetPermanentActorValueI(UInt32 actorValueCode);	// Manipulate GetPermanentActorValue, maybe convert to integer.
	virtual float	GetPermanentActorValue(UInt32 actorValueCode);	// GetPermanentActorValue (used from Eval) result in EAX
	virtual Actor*	Fn_09(void);					// GetActorBase (= this - 0x100) or GetActorBase (= this - 0x0A4)
	virtual UInt16	GetLevel(void);					// GetLevel (from ActorBase)
...
public:
	...
	TESIcon						icon;			// 05C
	TESScriptableForm			script;			// 068
	BGSMessageIcon				messageIcon;	// 0x74
	TESValueForm				value;			// 084
	TESWeightForm				weight;			// 08C
	UInt32						unk03C[(0xC4 - 0x94) / 4]; // 094
};
...
public:
	...
	void SetFlag(UInt32 pFlag, bool bMod)
	{
		flags = bMod ? (lightFlags | pFlag) : (lightFlags & ~pFlag);
	}
};
...
public:
	...
	TESIcon						icon;			// 05C
	TESScriptableForm			script;			// 068
	TESValueForm				value;			// 074
	TESWeightForm				weight;			// 07C
	UInt32 						unk084[2];		// 084
	BGSMessageIcon				messageIcon;	// 08C
	BGSPickupPutdownSounds		pickupPutdownSounds; // 09C
};
...
public:
	...
	BGSMessageIcon				messageIcon;	// 068
	TESValueForm				value;			// 078
	BGSPickupPutdownSounds		pickupPutdownSounds; // 080
...
public:
	...
	// Needs base value, pickupPickdownsound and Message icon
...
public:
	...
	union {
		UInt32					dataSound;				// 06C
		TESDescription			*dataText;				// 06C									
		TESIcon					*dataIcon;				// 06C
		struct {
			UInt32				topicID;				// 06C
			UInt32				speakerFormID;			// 070
		} dataVoice;
	};
	UInt32						unk074[(0x7C-0x74) >> 2];// 074 // Quest owners tList
	UInt8                       dataType;				// 07C
	UInt8                       read;					// 07D
	UInt8                       unk07E;					// 07E
	UInt8                       unk07F;					// 07F

	MEMBER_FN_PREFIX(BGSNote);
	DEFINE_MEMBER_FN(GetDataType, enum DataType, 0x5E8D40);
	DEFINE_MEMBER_FN(SetDataType, void, 0x5E8F90, enum DataType);
	DEFINE_MEMBER_FN(GetTextNote, TESDescription *, 0x5E91D0);
	DEFINE_MEMBER_FN(GetImageNote, TESIcon *, 0x5E9160);
	DEFINE_MEMBER_FN(GetSoundNote, UInt32, 0x5E9100);
	DEFINE_MEMBER_FN(GetVoiceNoteTopic, UInt32, 0x5E9240);
	DEFINE_MEMBER_FN(GetVoiceNoteSpeaker, TESNPC *, 0x5E92A0);
};

STATIC_ASSERT(offsetof(BGSNote, pickupPutdownSounds) == 0x60);
STATIC_ASSERT(offsetof(BGSNote, dataIcon) == 0x06C);
...
public:
	...
	void SetFlag(UInt32 pFlag, bool bMod)
	{
		flags = bMod ? (csFlags | pFlag) : (csFlags & ~pFlag);
	}
};
